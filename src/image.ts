...
export function drawRanking(order: string[]): HTMLCanvasElement {
  const height = HEADER + Math.min(order.length, PER_COL) * (ROW_H + ROW_GAP) + PAD
  ...
  ctx.fillText('My MLB team rankings', PAD, PAD + 24)

  ctx.fillStyle = DIM
  ctx.font = '16px ui-sans-serif, system-ui, -apple-system, sans-serif'
  ctx.fillText('All 30 teams, ranked one pick at a time', PAD, PAD + 50)
 ...
 */
export async function copyRankingImage(order: string[]): Promise<ImageResult> {
  const canvas = drawRanking(order)
